...
function displayContents(txt) {
    ...
    if(map[0].length<10 || map.length<10){
        cellsize=64;
    }
    else if((map[0].length>10 || map.length>10) && (map[0].length<100 || map.length<100)){
        cellsize=32;
    }
    else if((map[0].length>100 || map.length>100) && (map[0].length<500 || map.length<500)){
        cellsize=5;
    }
    else if((map[0].length>500 || map.length>500)){
        cellsize=2;
    }
    drawCanvas();
    drawGrid();
...
function drawGrid() {
    ...
    for (var row = 0; row < mapRows; row++) {
        ...
        for (var col = 0; col < mapCols; col++) {
            ctx.rect(sourceX, sourceY, cellsize, cellsize);
            ctx.stroke();
...
function markPath(current) {
    ...
    ctx.lineWidth=5;
    ctx.strokeRect(hashMap[(map[current[0]].length*current[0])+current[1]][0], hashMap[(map[current[0]].length*current[0])+current[1]][1], cellsize-1, cellsize-1);
}
